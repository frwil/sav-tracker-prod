...
export default function ProspectionForm() {
    ...
    const addActivity = () => {
        ...
        setTempAct({ spec: "Poulet de Chair", bat: 1, eff: 1000 }); // Reset default
    ...
    const handleSubmit = async () => {
        if (!formData.prospectName) return toast.error("Nom du prospect requis");
        ...
        const payload = {
            ...
            newPhotos: photos,
            technician: undefined // Sera géré par l'API via le token ou UserContext
        };
        ...
        try {
            ...
            toast.success("Prospection enregistrée ! 🚀");
            router.push("/dashboard");
        } catch (e) {
            // Repli hors ligne en cas d'erreur réseau
            addToQueue({ url, method, body: payload });
            toast("⚠️ Erreur réseau. Sauvegardé en local.");
            router.push("/dashboard");
        } finally {
        ...
        <div className="max-w-md mx-auto bg-white min-h-screen sm:min-h-0 sm:rounded-xl shadow-sm overflow-hidden flex flex-col">
                ...
                {step === 2 && (
                    <div className="space-y-4 animate-in slide-in-from-right">
                        ...
                        {activities.length > 0 ? (
                            ...
                            <p className="text-sm text-gray-400 italic text-center py-4">Aucune activité ajoutée pour l'instant.</p>
                        )}
                        ...
                        <div className="bg-gray-50 p-3 rounded-lg border border-gray-200 space-y-3">
                            ...
                            <div className="flex gap-2">
                                ...
                                <div className="flex-1">
                                    <label className="text-[10px] text-gray-400 uppercase">Effectif Total</label>
                                    <input 
                                        type="number" className="w-full p-2 border rounded text-sm h-[44px]"
                                        value={tempAct.eff}
                ...
                {step === 4 && (
                    <div className="space-y-6 animate-in slide-in-from-right">
                        ...
                        <button 
                            ...
                            className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-black py-4 rounded-xl shadow-lg transition active:scale-95"
                        >
            ...
            <div className="p-4 border-t bg-gray-50 flex gap-3">
                ...
                {step < 4 && (
                    <button 
                        onClick={() => setStep(step + 1)}
                        disabled={step === 1 && !formData.prospectName}
                        className={`flex-1 py-3 font-bold rounded-lg shadow-sm text-white
                            ${step === 1 && !formData.prospectName ? 'bg-gray-300 cursor-not-allowed' : 'bg-indigo-600'}
                        `}
                    >
                        Suivant
                    </button>
                ...
                <button onClick={()=> router.push('/dashboard/prospections')}
                    className="flex-1 py-3 bg-gray-50 border border-gray-100 text-gray-800 font-bold rounded-lg shadow-sm">Retour</button>
            </div>
        </div>
    );
}