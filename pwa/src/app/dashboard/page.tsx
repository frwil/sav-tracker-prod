...
const StatCard = ({ label, value, subValue, icon, color, loading, isPercent, alert, pending }: any) => (
            ...
            <div className="flex items-end gap-2 mb-1">
                {loading ? (
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        {pending > 0 && (
                            <span className="text-[10px] bg-yellow-100 text-yellow-800 px-1.5 py-0.5 rounded-full font-bold border border-yellow-200 animate-pulse print:hidden" title="Données en attente de synchronisation">
                                +{pending} ⏳
                            </span>
                        )}
                    </div>
    ...
    const renderStatsSection = (data: StatsData, isMain = false) => (
        <div className={`mb-8 break-inside-avoid ${isMain ? '' : 'opacity-90 scale-95 origin-left'}`}>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-4">
                ...
                <StatCard label="Cheptel Actif" value={data.activeFlocks.toString()} subValue="Lots en cours" loading={data.loading} icon="🐣" color="indigo" pending={data.pendingActive} />

                {/* 4. Santé du Parc (Avec Optimisme) */}
                <StatCard label="Alertes Santé" value={data.healthAlerts.toString()} subValue="Observations avec problèmes" loading={data.loading} alert={data.healthAlerts > 0} icon="❤️‍🩹" color={data.healthAlerts > 0 ? "red" : "green"} pending={data.pendingHealthAlerts} />
            
            </div>
            
            {/* Seconde ligne */}
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                 <StatCard label="Visites Tot." value={data.visitsCount.toString()} loading={data.loading} icon="📝" color="gray" pending={data.pendingVisits} />
