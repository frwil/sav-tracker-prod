...
export interface ProphylaxisTask {
    ...
    name: string;
    type: string;
    speculation?: string | { "@id"?: string; id?: number };
}
...
export const calculateBenchmark = (
    ...
    const weightRatio = !isNaN(currentWeight)
    ...
    if (!isNaN(currentFeed) && targetFeed > 0) {
        ...
        if (feedRatio >= 0.9 && feedRatio <= 1.1) feedStatus = "success";
        else if (feedRatio > 0.8 && feedRatio < 1.2) feedStatus = "warning";
...
export const estimateTotalFeedConsumption = (flock: Flock): number => {
    ...
    const sortedObs = [...flock.observations].sort(
        (a, b) =>
    ...
    for (const obs of sortedObs) {
        const currentAge =
            obs.data.age ||
            calculateAgeInDays(flock.startDate, obs.observedAt || "");
        ...
        if (daysDuration > 0 && currentConso > 0) {
            ...
            const periodConsumptionKg =
                (avgConsoG * avgSubjects * daysDuration) / 1000;
...
export const getFieldFeedback = (field: string, val: any) => {
    // ✅ Changé string en any
    // ✅ Conversion en string sécurisée
    const strVal = val != null ? String(val) : "";

    if (!strVal) return { style: "border-gray-300", message: null };

    if (field === "litiere") {
        if (
            strVal.includes("Détrempée") ||
            strVal.includes("Collante") ||
            strVal.includes("Croûteuse")
        )
            return {
                style: "border-red-500 bg-red-50 text-red-900 font-bold",
                message: "🚨 Risque Coccidiose",
            };
        if (strVal.includes("Humide"))
            return {
                style: "border-orange-500 bg-orange-50 text-orange-900 font-bold",
                message: "⚠️ Fermentation",
            };
        if (strVal.includes("Sèche") || strVal.includes("Friable"))
            return {
    ...
    if (field === "phValue") {
        // ✅ Gestion des valeurs numériques et string
        const numVal = parseFloat(strVal);

        // Si c'est un nombre valide
        if (!isNaN(numVal)) {
            if (numVal < 6 || numVal > 8.5)
                return {
                    style: "border-red-500 bg-red-50 text-red-900 font-bold",
                    message: "🚨 Danger : Traitement eau requis.",
                };
            if (
                (numVal >= 6 && numVal < 6.5) ||
                (numVal > 7.5 && numVal <= 8.5)
            )
                return {
                    style: "border-orange-500 bg-orange-50 text-orange-900 font-bold",
                    message: "⚠️ À surveiller.",
                };
            if (numVal >= 6.5 && numVal <= 7.5)
                return {
                    style: "border-green-500 bg-green-50 text-green-900 font-bold",
                    message: "✅ Eau OK.",
                };
        ...
        // Fallback sur les strings (ancien comportement)
        if (
            strVal.includes("Danger") ||
            strVal.includes("< 6") ||
            strVal.includes("> 8")
        )
            return {
        ...
        if (strVal.includes("Acceptable"))
            return {
                style: "border-orange-500 bg-orange-50 text-orange-900 font-bold",
                message: "⚠️ À surveiller.",
            };
        if (strVal.includes("Optimal") || strVal.includes("Bon"))
            return {
                style: "border-green-500 bg-green-50 text-green-900 font-bold",
                message: "✅ Eau OK.",
            };
    ...
    if (field === "uniformite") {
        if (strVal.includes("Mauvais") || strVal.includes("< 60"))
            return {
                style: "border-red-500 bg-red-50 text-red-900 font-bold",
                message: "🚨 Hétérogénéité critique.",
            };
        if (strVal.includes("Moyen") || strVal.includes("60%"))
            return {
                style: "border-orange-500 bg-orange-50 text-orange-900 font-bold",
                message: "⚠️ Tri nécessaire.",
            };
        if (strVal.includes("Bon") || strVal.includes("Excellent"))
            return {
                style: "border-green-500 bg-green-50 text-green-900 font-bold",
                message: "✅ Lot homogène.",
            };
    }

    if (field === "cv") {
        if (strVal.includes("> 12"))
            return {
                style: "border-red-500 bg-red-50 text-red-900 font-bold",
                message: "🚨 Lot très hétérogène.",
            };
        if (strVal.includes("10 - 12"))
            return {
                style: "border-orange-500 bg-orange-50 text-orange-900 font-bold",
                message: "⚠️ À surveiller.",
            };
        if (strVal.includes("< 8") || strVal.includes("8 - 10"))
            return {
                style: "border-green-500 bg-green-50 text-green-900 font-bold",
                message: "✅ Lot homogène.",
            };
...
export const getWaterOptions = (speculationName: string) => {
    const isFish =
        speculationName?.toLowerCase().includes("pisciculture") ||
        speculationName?.toLowerCase().includes("poisson");
    if (isFish)
        return [
            "6.5 - 7.5 (Optimal)",
            "6.0 - 6.5 (Acceptable)",
...
export const getPreviousWeight = (
    ...
    if (!flock.observations || flock.observations.length === 0) return 0;
    const history = flock.observations.filter(
        (o) => o.id !== currentObsId && o.data?.poidsMoyen > 0,
    );
...
export const getHistoricalObservations = (
    flock: Flock,
    currentObsId?: number,
): Observation[] => {
 ...
 */
export const generateExpertInsights = (
    obs: any,
    flock: any,
    benchmark: any,
    density: number,
    ...
    if (mortalitePourcentage > 5) {
        insights.push({
            type: "danger",
            text: `Mortalité cumulée critique : ${mortalitePourcentage.toFixed(1)}% (> 5%).`,
        });
    }
    ...
    if (isPoultry) {
        if (obs.data.abreuvoirs > 0) {
            ...
            if (ratioAbr > 80)
                insights.push({
                    type: "danger",
                    text: `Manque d'abreuvoirs : 1 pour ${ratioAbr.toFixed(0)} sujets (Max 80).`,
                });
        }
        if (obs.data.mangeoires > 0) {
            ...
            if (ratioMang > 55)
                insights.push({
                    type: "danger",
                    text: `Manque de mangeoires : 1 pour ${ratioMang.toFixed(0)} sujets (Max 55).`,
                });
        }
        if (density > 22)
            insights.push({
                type: "danger",
                text: `Surdensité critique : ${density} suj/m² (Risque étouffement/chaleur).`,
            });
        else if (density > 18)
            insights.push({
                type: "warning",
                text: `Densité élevée : ${density} suj/m².`,
            });
    }
    ...
    if (isPig) {
        if (density > 1.5)
            insights.push({
                type: "warning",
                text: `Densité porcine suspecte : ${density} suj/m². Vérifiez l'espace vital.`,
            });
    }
    ...
    if (phValue != null) {
        // ✅ Vérifie null/undefined
        // ✅ Conversion en string pour includes()
        const phStr = String(phValue);

        // Si c'est un nombre, on analyse aussi la valeur numérique
        const phNum = parseFloat(phValue);

        if (!isNaN(phNum)) {
            // Analyse numérique
            if (phNum < 6.0 || phNum > 8.5) {
                insights.push({
                    type: "danger",
                    text: "Qualité d'eau critique : Risque de diarrhées ou d'inefficacité des traitements.",
                });
            } else if (
                (phNum >= 6.0 && phNum < 6.5) ||
                (phNum > 7.5 && phNum <= 8.5)
                ...
                insights.push({
                    type: "warning",
                    text: "Qualité d'eau acceptable mais à surveiller.",
                });
            }
        } else {
            // Fallback analyse string (ancien comportement)
            if (
                phStr.includes("Danger") ||
                phStr.includes("< 6") ||
                phStr.includes("> 8.5")
            ) {
                insights.push({
    ...
    if (inv) {
        if (obs.data.feedStrategy === "SELF_MIX") {
            if (inv.mais && inv.mais.current < 100)
                insights.push({
                    type: "warning",
                    text: "Stock Maïs faible (< 100kg).",
                });
            if (inv.concentre && inv.concentre.current < 20)
                insights.push({
                    type: "danger",
                    text: "Rupture de Concentré imminente !",
                });
        } else if (inv.complete) {
            if (inv.complete.current < 50)
                insights.push({
                    type: "danger",
                    text: "Stock d'aliment critique (< 50kg).",
                });
            else if (inv.complete.current < 150)
                insights.push({
                    type: "warning",
                    text: "Prévoir commande d'aliment.",
                });
        }
    }
    ...
    if (
        ...
        !feedBrand
            .toLowerCase()
            .includes(currentStage.toLowerCase().split(" ")[0])
    ) {
    ...
    if (benchmark) {
        ...
        if (benchmark.weightStatus === "danger") {
            const gapPct =
                benchmark.targetWeight > 0
                    ? Math.abs(
                          (benchmark.weightGap / benchmark.targetWeight) * 100,
                      ).toFixed(0)
                    : 0;
            insights.push({
                type: "danger",
                text: `Retard de croissance important : -${gapPct}% par rapport à l'objectif.`,
            });
        }
        ...
        if (benchmark.feedStatus === "danger" && benchmark.feedGap < 0) {
            insights.push({
                type: "danger",
                text: `Sous-consommation d'aliment (${benchmark.feedGap.toFixed(0)}g/j). Vérifiez la santé ou la température.`,
            });
        } else if (benchmark.feedGap > 20) {
            insights.push({
                type: "warning",
                text: `Gaspillage d'aliment suspect (+${benchmark.feedGap.toFixed(0)}g/j). Vérifiez le réglage des mangeoires.`,
            });
        }
        ...
        if (currentStage === "Pré-démarrage" && benchmark.feedGap < -2) {
            insights.push({
                type: "danger",
                text: "🚨 Sous-consommation critique en pré-démarrage : Risque de retard irréversible.",
            });
        }
    }
    ...
    if (isPoultry) {
        ...
        if (uniformite) {
            ...
            if (uniformite.includes("Mauvais") || (valNum > 0 && valNum < 70)) {
                ...
                if (age <= 7)
                    diagnostic +=
                        " Problème de démarrage (préchauffage/qualité poussin).";
                else if (age > 21)
                    diagnostic +=
                        " Stress thermique ou densité excessive suspectée.";
                else
                    diagnostic +=
                        " Vérifiez l'accès égal à l'aliment (nombre/hauteur mangeoires).";

                insights.push({ type: "danger", text: `🚨 ${diagnostic}` });
            } else if (
                uniformite.includes("Moyen") ||
                (valNum >= 70 && valNum < 80)
            ) {
                insights.push({
                    type: "warning",
                    text: "⚠️ Uniformité moyenne : Surveillez la compétition aux mangeoires.",
                });
            }
        }
    }
    ...
    if (obs.data.waterConsumptionIncrease === "no") {
        insights.push({
            type: "danger",
            text: "↘️ Chute de consommation d'eau : Premier signe de maladie.",
        });
    }

    if (
        obs.data.litiere &&
        (obs.data.litiere.includes("Détrempée") ||
            obs.data.litiere.includes("Croûteuse"))
    ) {
        insights.push({
            type: "danger",
            text: "Litière dégradée : Risque élevé de coccidiose et brûlures de pattes.",
        });
    }
...
export const BenchmarkCard = ({
    ...
    const target =
        type === "weight" ? benchmark.targetWeight : benchmark.targetFeed;
    if (type === "feed" && (!target || target === 0)) return null;

    const status =
        type === "weight" ? benchmark.weightStatus : benchmark.feedStatus;
    ...
    return (
        <div
            className={`mt-1 px-2 py-1 rounded text-[10px] flex justify-between items-center ${s.bg}`}
        >
